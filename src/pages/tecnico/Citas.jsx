...
export default function Citas() {
  ...
  const [filtro,  setFiltro]  = useState('hoy') // 'hoy' | 'todas'

  const cargarCitas = useCallback(() => {
    const API = import.meta.env.VITE_API_URL
    setLoading(true)

    const url = filtro === 'hoy'
      ? `${API}/tecnico/citas?fecha=${fecha}`
      : `${API}/tecnico/citas`
      ...
      <div className="citas-page">
        ...
        <div className="citas-hero">
          <div className="citas-hero-text">
            ...
            <h1>Mis Citas</h1>
            <p>Gestiona tus servicios asignados y actualiza su estado.</p>
          </div>
          <div className="citas-hero-badge">
            {loading ? '…' : citas.length}
            <span>cita{citas.length !== 1 ? 's' : ''}</span>
          </div>
        </div>
        ...
        <div className="citas-filters">
          <div className="citas-tabs">
            <button
              className={`citas-tab ${filtro === 'hoy' ? 'active' : ''}`}
              onClick={() => setFiltro('hoy')}
            >
              Hoy
            </button>
            <button
              className={`citas-tab ${filtro === 'todas' ? 'active' : ''}`}
        ...
        {!loading && !error && citas.length === 0 && (
          <div className="citas-empty">
            ...
            <h3>Sin citas {filtro === 'hoy' ? 'para este día' : 'asignadas'}</h3>
                ...
                <div className="cita-card" key={cita.id}>
                  <div className="cita-card-left">
                    ...
                    <div className="cita-meta">
                      <span>
                        <UserIcon />
                        {cita.cliente_nombre} {cita.cliente_paterno}
                      </span>
                      <span>
                        <DevIcon />
                        {DISP_LABEL[cita.dispositivo_tipo] || cita.dispositivo_tipo}
                      </span>
                      <span>
                        <MapIcon />
                        {cita.colonia}, {cita.delegacion}
                      </span>
                      {cita.fecha_estimada && (
                        <span>
                          <CalIcon />
                          {formatFecha(cita.fecha_estimada)} · {formatHora(cita.fecha_estimada)}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <button
                    className="cita-btn-detalle"
                    onClick={() => navigate(`/tecnico/citas/${cita.id}`)}
                  >
                    Ver detalle
                    <ArrowIcon />
                  </button>
